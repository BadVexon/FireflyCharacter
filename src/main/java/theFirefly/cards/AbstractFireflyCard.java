...
import static theFirefly.FireflyMod.getModID;
import static theFirefly.FireflyMod.makeCardPath;

public abstract class AbstractFireflyCard extends CustomCard {
    ...
    public boolean upgradedQuantum;
    public boolean isQuantumModified;

    public AbstractFireflyCard(final String id, final int cost, final CardType type, final CardRarity rarity, final CardTarget target) {
        super(id, "ERROR", getCorrectPlaceholderImage(id), //makeCardPath(id.replaceAll(getModID(), "")) + ".png"
                cost, "ERROR", type, TheFirefly.Enums.FIREFLY_CYAN, rarity, target);
    ...
    public AbstractFireflyCard(final String id, final int cost, final CardType type, final CardRarity rarity, final CardTarget target, final CardColor color) {
    ...
    public void displayUpgrades() {
        ...
        if (upgradedQuantum) {
            quantum = baseQuantum;
            isQuantumModified = true;
        }
    }

    void upgradeQuantum(int amount) {
        baseQuantum += amount;
        quantum = baseQuantum;
        upgradedQuantum = true;
    }


    private static String getCorrectPlaceholderImage(String id) {
        return makeCardPath(id.replaceAll((FireflyMod.getModID() + ":"), "")) + ".png";
    }
    ...
    void shuffleIn(AbstractCard c, int i) {
        atb(new MakeTempCardInDrawPileAction(c, i, true, false));
    }
