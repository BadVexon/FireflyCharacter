...
public class SpyglassPatch {
    ...
    public static class MapRoomNodeRender {
        public static void Postfix(MapRoomNode self, SpriteBatch sb) {
            if (!(AbstractDungeon.floorNum == 0 && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.EVENT) && self.hb.hovered && self.room instanceof EventRoom && AbstractDungeon.player.hasRelic(LongRangeScanner.ID) && !LongRangeScanner.nodeList.containsKey(getNodeIndex(self))) {
                ...
                LongRangeScanner.nodeList.put(getNodeIndex(self), result);
            ...
            } else if (!(AbstractDungeon.floorNum == 0 && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.EVENT) && self.hb.hovered && self.room instanceof EventRoom && AbstractDungeon.player.hasRelic(LongRangeScanner.ID) && LongRangeScanner.nodeList.containsKey(getNodeIndex(self))) {
                ArrayList<PowerTip> tips = new ArrayList<>();
                EventHelper.RoomResult result = LongRangeScanner.nodeList.get(getNodeIndex(self));
                tips.add(new PowerTip("? Room Contents", "This ? room contains a(n) " + result.name().toLowerCase() + "."));
                TipHelper.queuePowerTips(self.hb.x + self.hb.width + 20.0f * Settings.scale, self.hb.y + self.hb.height, tips);
            }
        }
